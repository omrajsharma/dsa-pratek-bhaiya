#include <iostream>
#include <queue>
#include <vector>
#include <algorithm>
using namespace std;

/*
building of heap O(k)
pop of k elements O((n-k)logK)

INPUT :
5 3 
C1 1 1
C2 2 1
C3 3 2
C4 0 1
C5 2 3
*/

class Car{
public:
    string id;
    int x,y;
    Car(string id, int x, int y){
        this->id = id;
        this->x = x;
        this->y = y;
    }

    int dist() const {
        return x*x + y*y;
    }
};

// comparison class
class CarCompare{

public:
    // custom comparison operator
    bool operator()(const Car A, const Car B){
        // for max heap
        return A.dist() < B.dist();
    }
    
};

bool distComp(Car A, Car B){
    return A.dist() < B.dist();
}

// SOLUTION
void printNearestCars(vector<Car> cars, int k){

    // create priority max heap
    priority_queue< Car, vector<Car>, CarCompare > max_heap( cars.begin(), cars.begin() + k );

    // remaining cars
    for(int i=k; i<cars.size(); i++){
        auto car = cars[i];

        if(car.dist() < max_heap.top().dist()){
            max_heap.pop();
            max_heap.push(car);
        }
    }

    // creating the k nearest cars
    vector<Car> output;
    while(!max_heap.empty()){
        // cout << max_heap.top().id << endl;
        output.push_back(max_heap.top());
        max_heap.pop();
    }

    sort(output.begin(), output.end(), distComp);
    // or reverse will work same

    for(auto car : output){
        cout << car.id << endl;
    }

    return;
}

int main(){
    int N, K;
    cin >> N >> K;

    string id;
    int x,y;

    vector<Car> cars;

    for(int i=0; i<N; i++){
        cin >> id >> x >> y;
        Car car(id,x,y);
        cars.push_back(car);
    }

    printNearestCars(cars, K);

    return 0;
}